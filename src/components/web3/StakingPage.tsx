    ...
    <div className="space-y-8">
      <PageHeader
        ...
        actions={
          <Button
            ...
            className="btn-poly gap-2 rounded-xl h-10 px-5"
          >
      ...
      {isLoading ? (
        ...
        >
          <motion.div variants={itemVariants}>
            <StatsCard icon={Coins} label={t('total_staked')} value={`$${totalStaked.toLocaleString()}`} />
          </motion.div>
          <motion.div variants={itemVariants}>
          ...
          <motion.div variants={itemVariants}>
            <StatsCard icon={Clock} label={t('pending_rewards_label')} value={`$${totalPendingRewards.toFixed(2)}`} />
          </motion.div>
          <motion.div variants={itemVariants}>
            <StatsCard icon={Shield} label={t('active_stakes_label')} value={`${summary?.activeStakesCount ?? 0}`} />
          </motion.div>
        </motion.div>
      )}
      ...
      <div>
        <div className="flex items-center gap-3 mb-5">
          <div className="p-1.5 rounded-lg bg-[#8247E5]/10">
            <Flame className="h-5 w-5 text-[#8247E5]" />
          </div>
          ...
          >
            {plans.map((plan, index) => (
              ...
              >
                ...
                >
                  <div className="glass-card rounded-2xl h-full hover:border-[#8247E5]/30 transition-all duration-300">
                    <CardContent className="p-5 sm:p-6">
                      <div className="space-y-5">
                        {/* Header */}
                        <div className="flex items-center justify-between">
                          <h3 className="text-lg font-bold text-white group-hover:text-[#9B6DFF] transition-colors duration-300">
                            {plan.name}
                          </h3>
                          <Badge
                            className="border-[#8247E5]/30 text-[#8247E5] bg-[#8247E5]/8 text-xs font-medium"
                            variant="outline"
                          >
                        ...
                        <div className="text-center py-4 relative">
                          <div className="absolute inset-0 bg-[#8247E5]/3 rounded-xl" />
                          <div className="relative">
                            <p className="text-xs text-gray-500 mb-1 uppercase tracking-wider font-medium">
                              {t('daily_rate')}
                            </p>
                            <span className="text-5xl sm:text-6xl font-black text-gradient-poly leading-none">
                              {(plan.apy / 365).toFixed(2)}%
                            </span>
                            <p className="text-sm text-[#9B6DFF] font-semibold mt-1">
                              {t('per_day')}
                            </p>
                            {/* APY - secondary info */}
                            <div className="mt-2 pt-2 border-t border-[#8247E5]/10">
                        ...
                        <div className="space-y-2.5 text-sm">
                          ...
                          <div className="w-full h-px bg-gradient-to-r from-transparent via-[#8247E5]/15 to-transparent" />
                          <div className="flex justify-between items-center">
                        ...
                        {/* CTA Button */}
                        <Button className="btn-poly w-full rounded-xl h-11 text-sm">
      ...
      <div>
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            action={
              <Button onClick={() => setDepositOpen(true)} className="btn-poly gap-2 rounded-xl">
                <Plus className="h-4 w-4" />
                Start Staking
              </Button>
            }
              ...
              return (
                <motion.div
                  ...
                  className={`
                    ...
                    border-l-[3px]
                    ${isActive ? 'border-l-[#8247E5]' : isWithdrawn ? 'border-l-gray-600' : 'border-l-amber-500'}
                    hover:border-[#8247E5]/30
                  `}
                >
                      ...
                      <div className="space-y-3 flex-1">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <Badge
                            className={`
                              text-xs font-medium
                              ${isActive
                                ? 'bg-[#8247E5]/10 text-[#8247E5] border-[#8247E5]/20'
                                : isWithdrawn
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                          ...
                          <div>
                            ...
                            <p className="text-[#9B6DFF] font-semibold">${Number(stake.pendingRewards).toFixed(2)}</p>
                          </div>
                          <div>
                        ...
                        {isActive && (
                          <div className="space-y-1.5">
                            <div className="flex justify-between text-xs">
                              ...
                              <span className="text-[#8247E5] font-medium">{progress.toFixed(1)}%</span>
                            </div>
                            <div className="h-2.5 w-full bg-gray-800/60 rounded-full overflow-hidden">
                              <motion.div
                                className="h-full rounded-full bg-gradient-to-r from-[#6B33D4] via-[#8247E5] to-[#9B6DFF]"
                                initial={{ width: 0 }}
                      ...
                      {isActive && (
                        <div className="flex sm:flex-col gap-2 shrink-0">
                          <Button
                            ...
                            className="bg-[#8247E5]/15 hover:bg-[#8247E5]/25 text-[#8247E5] border border-[#8247E5]/20 gap-1.5 rounded-xl flex-1 sm:flex-none h-9 font-medium transition-all"
                          >
      ...
      <Dialog open={depositOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="glass-strong border-[#8247E5]/15 text-white sm:max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-xl text-gradient-poly text-3xl font-black">New Stake</DialogTitle>
            <DialogDescription className="text-gray-400">
              Stake USDT to earn rewards on Polygon
            </DialogDescription>
          </DialogHeader>
                ...
                <div key={s.step} className="flex items-center">
                  <div className="flex flex-col items-center gap-1">
                    <div className={`
                      ...
                      ${status === 'done'
                        ...
                        : status === 'active'
                        ? 'bg-[#8247E5]/10 text-[#8247E5] glow-poly'
                    ...
                    <span className={`text-[10px] font-medium ${
                      status === 'done' ? 'text-[#8247E5]' : status === 'active' ? 'text-[#9B6DFF]' : 'text-gray-600'
                    }`}>
                      {s.label}
                    </span>
                  </div>
                  {i < 2 && (
                    <div className={`w-10 sm:w-14 h-0.5 mx-1 mb-4 rounded-full transition-all duration-500 ${
                      getStepStatus(s.step + 1) !== 'pending' ? 'bg-[#8247E5]/40' : 'bg-gray-800'
                    }`} />
                  )}
                </div>
              )
          ...
          <div className="space-y-4 mt-2">
            <div className="space-y-2">
              ...
              <Select value={selectedPlan} onValueChange={setSelectedPlan}>
                <SelectTrigger className="bg-[#0a0a0f]/60 border-[#8247E5]/15 text-white focus:ring-[#8247E5]/30 focus:border-[#8247E5]/30 rounded-xl h-11">
                  <SelectValue placeholder="Choose a staking plan" />
                </SelectTrigger>
                <SelectContent className="bg-[#0f0f1a] border-[#8247E5]/15">
                  {plans.map(plan => (
                    <SelectItem key={plan.id} value={plan.id} className="text-white focus:bg-[#8247E5]/10 focus:text-[#9B6DFF]">
            ...
            {selectedPlanData && (
              <div className="p-3.5 rounded-xl glass border border-[#8247E5]/10 space-y-2 text-sm">
                <div className="flex justify-between items-center">
                  <span className="text-gray-400">{t('daily_rate')}</span>
                  <span className="text-[#8247E5] font-bold text-base">{(selectedPlanData.apy / 365).toFixed(2)}% <span className="text-xs font-normal text-gray-500">{t('per_day')}</span></span>
                </div>
                <div className="flex justify-between items-center">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  className="bg-[#0a0a0f]/60 border-[#8247E5]/15 text-white focus:ring-[#8247E5]/30 focus:border-[#8247E5]/30 rounded-xl h-11 pr-16"
                />
                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
                  <div className="h-4 w-4 rounded-full bg-[#8247E5]/20 flex items-center justify-center">
                    <span className="text-[8px] font-bold text-[#8247E5]">$</span>
                  </div>
                  <span className="text-xs text-gray-500 font-medium">USDT</span>
                </div>
              </div>
              {selectedPlanData && depositAmount && (
                <div className="p-3 rounded-xl bg-[#8247E5]/5 border border-[#8247E5]/15 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-400 flex items-center gap-1.5">
                      <Sparkles className="h-3.5 w-3.5 text-[#8247E5]" />
                      {t('daily_rate')}
                    </span>
                    <span className="text-lg font-bold text-gradient-poly">{(selectedPlanData.apy / 365).toFixed(2)}% <span className="text-xs font-normal text-[#9B6DFF]">{t('per_day')}</span></span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-gray-500">{t('estimated_daily')}</span>
                    <span className="text-[#9B6DFF] font-medium">~${((Number(depositAmount) * selectedPlanData.apy / 100) / 365).toFixed(2)}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
            ...
            {!approveStep && !confirmStep && (
              <Button
                ...
                className="btn-poly w-full rounded-xl h-11 text-sm disabled:opacity-40 disabled:cursor-not-allowed"
              >
            ...
            {approveStep && !confirmStep && (
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-sm text-[#8247E5] p-2 rounded-lg bg-[#8247E5]/5 border border-[#8247E5]/10">
                ...
                <Button
                  ...
                  className="btn-poly w-full rounded-xl h-11 text-sm"
                >
            ...
            {confirmStep && (
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-sm text-[#8247E5] p-3 rounded-xl bg-[#8247E5]/5 border border-[#8247E5]/15 glow-poly">
                  <CheckCircle2 className="h-5 w-5 shrink-0" />
                  <div>
                    <p className="font-semibold text-[#9B6DFF]">Transaction Confirmed!</p>
                    <p className="text-gray-400 text-xs mt-0.5">
                      Your stake is now active on Polygon. Rewards accrue immediately.
                    </p>
                  </div>
                </div>
                <Button
                  onClick={resetDialog}
                  className="w-full bg-[#0a0a0f]/60 border border-[#8247E5]/15 text-[#8247E5] hover:bg-[#8247E5]/10 hover:border-[#8247E5]/25 rounded-xl h-11 font-medium transition-all"
                >
                  Done
                </Button>
              </div>
